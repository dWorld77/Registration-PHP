                ...
                if (mysqli_num_rows($result) == 1) 
                {
                    ...
                    if($row["auth_status"] == "created" || $row["auth_status"] == "opened")
                    {
                        ...
                        if (isset($_POST['register'])) 
                        {
                            ...
                            if ($rowCount == 1)
                            { 
                                $query = "UPDATE registration SET student_name = '$name', college_name = '$college', event_name = '$event',  auth_status = 'closed' WHERE auth_token = '$auth_req'";
        ...
        <div class="container">
            <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . "?auth=" . $auth_req; ?>" method="post">
                <div class="form-group">
                    <label for="name" style="font-weight: bold;">Student Name:</label>
                    <input type="text" name="name" required>
                </div>
                <div class="form-group">
                    <label for="event" style="font-weight: bold;">Event:</label>
